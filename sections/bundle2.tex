...
\subsubsection{Problem Definition}
High false-alarm rates are not just a technical inconvenience --- they are a well-documented operational failure that has real consequences in deployed surveillance systems~\cite{deepsurvey2023anomaly}. When operators are flooded with spurious alerts day after day, two things happen. They begin to distrust the system, and they burn through response resources chasing triggers that turn out to be nothing. The underlying causes are varied. Environmental noise from wind-driven fence vibration, animals crossing sensor zones, and weather-induced radar clutter all contribute. Beyond that, most threshold-based alarm systems have no mechanism for checking whether a detected anomaly persists over time before raising an alert, and they carry no awareness of context --- a sensor trip near a busy civilian crossing and one near a restricted military perimeter are treated identically~\cite{cctv2024anomaly}.

\subsubsection{Existing Approaches}
Weakly-supervised multiple-instance learning frameworks for anomaly detection in real-world surveillance video have offered a practical path forward, significantly reducing the annotation burden required for large-scale deployment~\cite{deepsurvey2023anomaly}. Broader deep learning surveys document CNN, LSTM, and transformer-based temporal models that improve anomaly discrimination by requiring a detected signature to persist across consecutive frames before any alert is raised, rather than reacting to a single frame in isolation. Hybrid architectures that combine spatial object detection with temporal verification have demonstrated measurable reductions in false-positive rates compared to single-frame classifiers in CCTV monitoring contexts~\cite{cctv2024anomaly}. On the explainability side, Zhang et al.~\cite{zhang2022xai} survey SHAP, LIME, and attention-based explanation mechanisms for black-box classifiers, applying them to intrusion detection and malware classification tasks that share structural similarities with the threat-triage function needed in military surveillance. Roff and Danks~\cite{military2024xai} take a different angle, focusing on the accountability obligations specific to military environments and arguing that commanders must be able to audit the reasoning behind any targeting or escalation recommendation under both domestic law and international humanitarian obligations.

\subsubsection{Key Insight and Research Gap}
The explainability work reviewed here has a consistent and important limitation. XAI methods in surveillance have almost exclusively been evaluated for post-hoc interpretability on static or pre-recorded datasets~\cite{zhang2022xai}. None of them have been integrated into live decision pipelines where the time it takes to generate an explanation directly limits how quickly an operator can respond. That is a very different problem from explaining a recorded event after the fact. Separately, contextual filtering approaches that suppress alarms in geofenced civilian zones rely on accurate, up-to-date civilian activity maps. In active border areas these maps are either unavailable or classified~\cite{cctv2024anomaly}, creating a data dependency that none of the reviewed studies have found a way around. Both of these remain open problems that a deployable real-time surveillance system must confront directly.
